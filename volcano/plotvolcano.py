    ...
    for i in range(0,df_size):
        ...
        if (float(attr['GeLiNEA p-value']) * geneset_size) < 0.5:
            h_df = h_df.append({'id' : in_json['elements'][i]['id'],
                                ...
                                'GeLiNEA_p_adj': float(attr['GeLiNEA p-value']) * geneset_size
                            },
                            ignore_index = True) 
    return h_df
...
class VolcanoApp:
    ...
    def headers_selector(self,handle=None):
        ...
        if handle == None:
            st.markdown('<p class="header-style">Headers</p>',unsafe_allow_html=True)
            col1, col2, col3, col4 = st.columns(4)
        else:
            handle.markdown('<p class="header-style">Headers</p>',unsafe_allow_html=True)
            col1, col2, col3, col4 = handle.columns(4)
    ...
    def construct_sidebar(self):
        ...
        form_volcano.markdown('<p class="header-style">Volcano and gene lists parameters</p>',unsafe_allow_html=True)
        ...
        adj = form_volcano.checkbox("Label auto-adjust - takes few minutes")

        submit_button_volcano = form_volcano.form_submit_button(label='Go!')

        form_gelinea = st.sidebar.form(key='form_gelinea')
        form_gelinea.markdown('<p class="header-style">Enrichment parameters</p>',unsafe_allow_html=True)
    ...
    def construct_app(self):
        ...
        f = form_file.file_uploader("Upload a file", type=(["tsv","csv","txt","tab","xlsx","xls"]))
